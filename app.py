...
app.config['UPLOAD_FOLDER'] = cur_path + r'\UPLOAD_FOLDER'
...
def main():
    if flask.request.method == 'POST':
        if 'src_tgt' not in flask.request.files:
            flask.flash('No file')
            return flask.redirect(flask.request.url)

        source_target_file = flask.request.files['src_tgt']
        mapped_file = flask.request.files['process_file']
